...
fs=16;
load Data\etkE5  %% eyetracking data in Experiment 5
...
%%
%blink, saccade, and pupil spectrum in 2 experimental conditions when eye is open in experiment 5
tnow=[t>=1 & t<21];TD=sum(tnow);
...
figure;
for idx=1:2
  [~,Afx0]=bootstrap_for_vector(Bv,0.05,@geomean,fx0(:,:,idx)');
...
%%
%The blink, saccade, and pupil response phase extracted by the Fourier analysisd in 2 experimental conditions when eye is open in experiment 5
DIM=11;
...
%%
%The blink, saccade, and pupil phase difference between conditions when eye is open in experiment 5
figure;
...
%%
%Response waveform(0.75 ~ 1.25 Hz) for blink, saccade, and pupil when eye is open in experiment 5
tg=[t>0 & t<21];
h=fir1(32,[.25 0.75]/8);
...
xlim([1 21]);ylim([0 .06]);set(gca,'xtick',0:2:22);grid on
...
xlim([1 21]);ylim([-500 300]+2000);set(gca,'xtick',0:2:22);grid on
