...
function PureThre(subjectid,noise_num)
sca;
try
    if nargin<1                              
        %如果没有输入参数的话
    ...
    %Stimuli_onsest = 0.5000;        % 纯音出现的时间（秒） 
    t_Pure = 0:1/Fs:(Duration_Pure-1/Fs); % 纯音时间点
    Fre_Pure=1e3;                   % 纯音的频率
    power_noise=[0.36, 0.64, 0.81, 1, 1.21];          % 每次生成的白噪音功率

    %% 实验开始
    for kk=[0 1]
        if kk==1        %% formal exp
            [paramatrix,~]=buildmatrix(noise_num);
        ...
        for k=1:length(paramatrix(:,1))
           %% 每80试次休息一下
            ...
            %% Define noise
            Noise_power=power_noise(1,Noise_index);
            ...
            white_noise =sqrt(Noise_power)*randn(1, Fs*Duration_Noise);
    ...
    Screen('Flip', window);
    fittingresult(subjectid,length(unique(paramatrix(:,2))),[]);
    ShowCursor;
    sca;
catch
    ShowCursor;
